...
const mapStateToProps = ({
	firebase,
	user,
}: IStoreState): ICryptoTradeStateProps => ({
	firebase,
	user,
});
...
const CryptoTrades: React.FC<ICryptoTradesQuery & ICryptoTradeStateProps> = ({
	firebase,
	user,
	data,
}) => {
	...
	const save = async (): Promise<void> => {
		...
		const docRef = trade
			? trade.docRef
			: firestore.collection('cryptoTrades').doc();
		...
		<Layout>
			...
			{trades.map((t) => (
				<div
					...
					key={`${t.symbol}${t.price}${t.quantity}`}
				>
					...
					<div className='col-2'>
						{!t.isSell && '-'}
						{numeral(t.price * t.quantity).format('$0,0.00')}
					</div>
					<div className='col-3'>
						{moment(t.timestamp.seconds * 1000).format(DATE_FORMAT)}
					</div>
				</div>
			))}
		</Layout>
